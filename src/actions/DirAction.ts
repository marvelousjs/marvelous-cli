...
export const DirAction: IAction<IProps> = async ({
  cliConfig,
  platformName,
  typeFilter,
  nameFilter
  ...
  console.log(chalk.bold('Package directories...'));

  await forEach(artifacts, async artifact => {
    const dir = path.join(homedir(), 'Developer', platformName, artifact.repo.name);

    console.log(`${artifact.repo.name} -> ${formatPath(dir)}`);
